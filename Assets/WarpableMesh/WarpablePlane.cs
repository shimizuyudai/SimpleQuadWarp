...
public class WarpablePlane : MonoBehaviour
{
    ...
    [SerializeField]
    bool isSelectable;
    RaySelectableObject raySelectableObject;
    ...
    private void Awake()
    {
        if (isSelectable)
        {
            ...
            raySelectableObject = gameObject.AddComponent<RaySelectableObject>();
            raySelectableObject.SelectedEvent += RaySelectableObject_SelectedEvent;
            raySelectableObject.ReleasedEvent += RaySelectableObject_ReleasedEvent;
        }
    ...
    private void RaySelectableObject_ReleasedEvent(RaySelectableObject obj)
    {
        this.IsEnable = false;
    }

    private void RaySelectableObject_SelectedEvent(RaySelectableObject obj)
    {
        this.IsEnable = true;
    }
    ...
    public void Init(Mesh mesh)
    {
        ...
        if (isSelfControl)
        {
            preMousePosition = cam.ScreenToWorldPoint(Input.mousePosition);
            preMousePosition.z = 0f;
        }
        HasInitialized = true;
        InitMeshCollider();
        refresh();
    ...
    {
        get {
            return CornerPoints.Select(e => new CornerPointInfomation(e.number, new Vec2(e.position.x, e.position.y))).ToArray();
        }
    }
